...
def loadGraphAndLabels(prefix):
  ...
  labels = loadNodeLabels(prefix + "_labels.csv")
  return G, labels
...
def testSlGraph():
  ...
  G, labels = loadGraphAndLabels(SIERRA_LEONE_PREFIX)
...
def testLibGraph():
  ...
  G, labels = loadGraphAndLabels(LIBERIA_PREFIX)
  checkGraph(G, labels, Gname, Gnodes, Gedges, Gdegs)
...
def testGuinGraph():
  ...
  G, labels = loadGraphAndLabels(GUINEA_PREFIX)
  checkGraph(G, labels, Gname, Gnodes, Gedges, Gdegs)
